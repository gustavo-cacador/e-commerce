...
public class UserControllerIT {
    ...
    // getMe retorna 200 qnd admin estiver logado
    @Test
    public void getMeShouldReturnUserDTOWhenAdminLogged() throws Exception {
        ...
        result.andExpect(jsonPath("$.nome").value("Alex Green"));
        result.andExpect(jsonPath("$.email").value("alex@gmail.com"));
        result.andExpect(jsonPath("$.telefone").value("977777777"));
        result.andExpect(jsonPath("$.dataNascimento").value("1987-12-13"));
        result.andExpect(jsonPath("$.roles").exists());
    }

    // getMe retorna 200 qnd cliente estiver logado
    @Test
    public void getMeShouldReturnUserDTOWhenClientLogged() throws Exception {
        ...
        result.andExpect(jsonPath("$.telefone").value("988888888"));
        result.andExpect(jsonPath("$.dataNascimento").value("2001-07-25"));
        result.andExpect(jsonPath("$.roles").exists());
    }

    // getMe retorna 401 qnd n for admin nem cliente
    @Test
